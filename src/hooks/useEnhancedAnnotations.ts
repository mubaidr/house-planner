...
export interface UseEnhancedAnnotationsReturn {
  ...
  dimensions: unknown[];
  chains: unknown[];
  createDimension: (startPoint: Point2D, endPoint: Point2D, options?: unknown) => unknown;
  updateDimension: (id: string, updates: unknown) => boolean;
  deleteDimension: (id: string) => boolean;
  autoGenerateDimensions: (elements: Element2D[]) => unknown[];
...
export function useEnhancedAnnotations(options: UseEnhancedAnnotationsOptions): UseEnhancedAnnotationsReturn {
  ...
  const importAnnotations = useCallback((data: unknown) => {
    ...
    if (data.areaAnnotations) setAreaAnnotations(data.areaAnnotations);
    if (data.materialCallouts) setMaterialCallouts(data.materialCallouts);
    if (data.styles) {
      if (data.styles.defaultTextStyle) setDefaultTextStyle(data.styles.defaultTextStyle);
      if (data.styles.defaultAreaStyle) setDefaultAreaStyle(data.styles.defaultAreaStyle);
      if (data.styles.defaultCalloutStyle) setDefaultCalloutStyle(data.styles.defaultCalloutStyle);
    }
  }, []);
  
  const updateDefaultStyles = useCallback((styles: unknown) => {
    if (styles.textStyle) setDefaultTextStyle(prev => ({ ...prev, ...styles.textStyle }));
    if (styles.areaStyle) setDefaultAreaStyle(prev => ({ ...prev, ...styles.areaStyle }));
    if (styles.calloutStyle) setDefaultCalloutStyle(prev => ({ ...prev, ...styles.calloutStyle }));
  }, []);
