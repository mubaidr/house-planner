...
export class RoofWallIntegrationSystem2D {
  ...
  private createRoofWallConnection(
    ...
    if (this.config.pitchCalculation.enabled) {
      try {
        ...
        console.warn('Could not calculate pitch data for roof-wall connection:', error);
      }
    }
  ...
  public updateConnectionPitch(connectionId: string, newPitch: number): boolean {
    ...
    try {
      ...
      if (!validation.valid) {
        console.warn('Invalid pitch:', validation.errors);
        return false;
      }
      ...
      console.error('Failed to update connection pitch:', error);
      return false;
    }
  }
