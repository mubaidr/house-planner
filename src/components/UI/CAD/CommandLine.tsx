...
const CAD_COMMANDS: CommandSuggestion[] = [
  { command: 'LINE', description: 'Draw a line', aliases: ['L'], parameters: ['start point', 'end point'] },
  { command: 'CIRCLE', description: 'Draw a circle', aliases: ['C'], parameters: ['center point', 'radius'] },
  { command: 'RECTANGLE', description: 'Draw a rectangle', aliases: ['REC', 'RECT'], parameters: ['first corner', 'opposite corner'] },
  { command: 'ARC', description: 'Draw an arc', aliases: ['A'], parameters: ['start point', 'second point', 'end point'] },
  { command: 'WALL', description: 'Draw a wall', aliases: ['W'], parameters: ['start point', 'end point', 'height'] },
  { command: 'DOOR', description: 'Insert a door', aliases: ['DR'], parameters: ['insertion point', 'width', 'height'] },
  { command: 'WINDOW', description: 'Insert a window', aliases: ['WI'], parameters: ['insertion point', 'width', 'height'] },
  { command: 'STAIR', description: 'Insert stairs', aliases: ['ST'], parameters: ['start point', 'end point', 'steps'] },
  { command: 'MOVE', description: 'Move objects', aliases: ['M'], parameters: ['base point', 'destination'] },
  { command: 'COPY', description: 'Copy objects', aliases: ['CO', 'CP'], parameters: ['base point', 'destination'] },
  { command: 'ROTATE', description: 'Rotate objects', aliases: ['RO'], parameters: ['base point', 'angle'] },
  { command: 'SCALE', description: 'Scale objects', aliases: ['SC'], parameters: ['base point', 'scale factor'] },
  { command: 'MIRROR', description: 'Mirror objects', aliases: ['MI'], parameters: ['first point', 'second point'] },
  { command: 'OFFSET', description: 'Offset objects', aliases: ['O'], parameters: ['offset distance', 'side to offset'] },
  { command: 'TRIM', description: 'Trim objects', aliases: ['TR'], parameters: ['cutting edges', 'objects to trim'] },
  { command: 'EXTEND', description: 'Extend objects', aliases: ['EX'], parameters: ['boundary edges', 'objects to extend'] },
  { command: 'FILLET', description: 'Create rounded corners', aliases: ['F'], parameters: ['radius', 'first object', 'second object'] },
  { command: 'CHAMFER', description: 'Create beveled corners', aliases: ['CHA'], parameters: ['distance1', 'distance2', 'first object', 'second object'] },
  { command: 'ZOOM', description: 'Zoom view', aliases: ['Z'], parameters: ['option (extents/window/scale)'] },
  { command: 'PAN', description: 'Pan view', aliases: ['P'], parameters: ['displacement'] },
  { command: 'LAYER', description: 'Manage layers', aliases: ['LA'], parameters: ['option (new/delete/current)'] },
  ...
  { command: 'EXPORT', description: 'Export drawing', aliases: ['EXP'], parameters: ['format', 'file path'] },
  ...
  { command: 'MEASURE', description: 'Measure distance', aliases: ['DIST'], parameters: ['first point', 'second point'] },
...
export function CommandLine({ height, onExecuteCommand, commandHistory, theme }: CommandLineProps) {
  ...
  useEffect(() => {
    ...
    if (currentCommand.trim()) {
      const filtered = CAD_COMMANDS.filter(cmd => 
  ...
  const executeCommand = useCallback((command: string) => {
    ...
    const cmdDef = CAD_COMMANDS.find(cmd => 
      ...
      case 'Enter':
        ...
        if (showSuggestions && suggestions.length > 0) {
          ...
          if (commandState === 'parameter') {
            ...
            const activeCmdDef = CAD_COMMANDS.find(cmd => 
              commandHistory.length > 0 && (
                cmd.command.toLowerCase() === commandHistory[commandHistory.length - 1].toLowerCase() ||
                cmd.aliases?.some(alias => alias.toLowerCase() === commandHistory[commandHistory.length - 1].toLowerCase())
              )
            );

            if (activeCmdDef && activeCmdDef.parameters && parameterIndex + 1 < activeCmdDef.parameters.length) {
              ...
              onExecuteCommand(`${commandHistory[commandHistory.length - 1]} ${newParams.join(' ')}`);
              setCommandState('waiting');
              setCurrentParameters([]);
              setParameterIndex(0);
              setCurrentCommand('');
  ...
  }, [currentCommand, showSuggestions, suggestions, selectedSuggestion, commandHistory, historyIndex, executeCommand, commandState, currentParameters, parameterIndex, onExecuteCommand, isExpanded]);
    ...
    >
      ...
      <div className="flex items-center h-full px-3 space-x-2">
        ...
        <input
          ...
          placeholder={commandState === 'waiting' ? 'Type a command or press Tab for suggestions' : ''}
          autoComplete="off"
          spellCheck={false}
        />
