...
import { wallsToPoints, sortPointsToPolygon, calculatePolygonArea } from '@/utils/math3D';
...
export function RoomCreationTool3D({ isActive, onDeactivate }: RoomCreationToolProps) {
  ...
  const handleWallClick = useCallback((wallId: string, event: ThreeEvent<MouseEvent>) => {
    if (!isActive) return;
  ...
  // Handle wall hover
  const handleWallHover = useCallback((wallId: string | null) => {
    if (!isActive) return;
          ...
          <bufferGeometry>
            <bufferAttribute
              ...
              array={new Float32Array([
                ...
                roomPoints[0].x, 0.1, roomPoints[0].y, // Close the loop
              ])}
              itemSize={3}
              args={[
                new Float32Array([
