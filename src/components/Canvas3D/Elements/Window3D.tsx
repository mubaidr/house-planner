...
export function Window3D({ windowId }: Window3DProps) {
  ...
  return (
    ...
    >
      {/* Window frame */}
      <mesh>
        <boxGeometry args={[windowElement.width + 0.1, windowElement.height + 0.1, windowElement.thickness + 0.05]} />
      ...
      {isSelected && (
        <mesh>
          <boxGeometry args={[windowElement.width + 0.15, windowElement.height + 0.15, windowElement.thickness + 0.1]} />
          <meshBasicMaterial 
            color="#3b82f6" 
            wireframe={true} 
            transparent={true} 
