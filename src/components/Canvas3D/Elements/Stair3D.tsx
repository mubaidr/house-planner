...
export function Stair3D({ stairId }: Stair3DProps) {
  ...
  const steps = useMemo(() => {
    ...
    const length = Math.sqrt(
      Math.pow(stair.end.x - stair.start.x, 2) + 
      Math.pow(stair.end.z - stair.start.z, 2)
    );
  ...
  return (
    ...
    >
      ...
      {isSelected && (
        <mesh>
          <boxGeometry args={[
            stair.stepDepth * stair.steps, 
            stair.stepHeight * stair.steps, 
            stair.width
          ]} />
          <meshBasicMaterial 
            color="#3b82f6" 
